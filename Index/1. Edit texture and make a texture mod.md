...
　　SB3UGS can delete textures and materials, but it can't delete regular animators, so that's why we need to choose texture assetbundle, but not the other files.
...
> 6. Change the CAB-string.Go to 'File'>'CABinest(s)'.Leave 'CAB-' structure but delete the other chars, then type in your randome chars.Or you could just rename your CAB-string like 'CAB-mytest'.
...
Here is an example about how to sign your eyeline.The ID must be unique so it won't conflict to the other item.Assign the same number to Kind and Poses to the other
